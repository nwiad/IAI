import gensim
import torch
from rnn import SentimentTextGRU
...
model = SentimentTextGRU(vocab_size=len(key2index), embedding=vectors, embedding_dim=EMBEDDING_DIM, 
                         hidden_dim=HIDDEN_DIM, layer_num=LAYER_NUM, output_dim=OUTPUT_DIM, dropout=DROPOUT).to(device)
...
for i in range(EPOCH):
    ...
    if i%3 == 0:
        ...
        accuracies = []
        with torch.no_grad():
            for labels, matrixes in validation_dataloader:
                prediction = model(matrixes)
                result = torch.max(prediction, dim=1)[1]
